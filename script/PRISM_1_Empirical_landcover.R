...
surveys <- unique(na.omit(survey_data[,c("PlotID","Year","Month","Day","Plot_Area_km2","Proportion_Surveyed","Sighting_Type")])) # n = 5581
surveys <- left_join(survey_coords,surveys) %>% na.omit() # Drops surveys that have no location information; n = 4835 remain
...
survey_count_df <- survey_data %>% 
  subset(Sighting_Type == "inside plot") %>%
...
for (species in species_list){
  ...
  sdat <- subset(sdat, exp(log_offset)>=0.1 & Proportion_Surveyed == 1)
  
  # --------------------------------
  # Visualizations of raw data for this species
  # --------------------------------
  
  # Relationship between survey effort and count
  ggplot() + geom_point(data = sdat, aes(x = log_offset, y = count/exp(log_offset)))
  
  # Examine data from 10 most abundant surveys
  subset(sdat, (count/exp(log_offset)) >= sort(sdat$count/exp(sdat$log_offset), decreasing = TRUE)[10]) %>%
    arrange(desc(count)) %>%
    dplyr::select(PlotID, Year, Month, Day, Plot_Area_km2, Proportion_Surveyed, count) %>%
    mutate(count_per_km2 = count/Plot_Area_km2)
  
  hexsurveys <- table(sdat$hexid) %>% sort(decreasing = TRUE)
  
  hex_summary <- sdat %>% as.data.frame() %>% group_by(hexid) %>% summarize(n_det = sum(count>0)) %>%arrange(desc(n_det))
  
  # Plot data in hexagons that have more than 10 detections
  ggplot()+geom_sf(data = subset(sdat, hexid == hex_summary$hexid[2] & count > 0),
                   aes(size = exp(log_offset), col = count/exp(log_offset)))
  ...
  prior_sigma <- c(0.1,0.1)         # 1% chance sd is larger than 1
  ...
  comps <- ~
    ...
    PC2_beta2(PC2^2, model = "linear", mean.linear = 0, prec.linear = 100)+
    
    PC3_beta1(PC3, model = "linear", mean.linear = 0, prec.linear = 100)+
    PC3_beta2(PC3^2, model = "linear", mean.linear = 0, prec.linear = 100)
  ...
  model_formula_count = count ~ intercept_count + 
    spde_count + 
    surv_effect + 
    plot_effect + 
    log_offset +
