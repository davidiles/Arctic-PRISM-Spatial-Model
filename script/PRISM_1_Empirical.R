...
for (species in species_list){
  ...
  if (file.exists("../output/results_empirical.RDS")) results <- readRDS("../output/results_empirical.RDS")
  if (nrow(results) > 0 & species %in% results$species) next
  
  sdat <- surveys %>% mutate(count = count_matrix[,species])
  ...
  survey_map <- species_plots[[species]]
  ...
  model_formula_count = count ~ intercept_count + spde_count + surv_effect + plot_effect + log_offset
  ...
  if (file.exists("../output/maps_empirical.RDS")) maps <- readRDS("../output/maps_empirical.RDS")

  saveRDS(maps,"../output/maps_empirical.RDS")
  ...
  saveRDS(results,"../output/results_empirical.RDS")
