...
length(species_list) # 29 species
...
for (species in species_list){
  ...
  if (file.exists("../output/empirical_results_RE2.RDS")) results <- readRDS("../output/empirical_results_RE2.RDS")
  ...
  ggplot()+geom_sf(data = subset(sdat, count > 0),
  ...
  comps <- ~
    ...
    #surv(surv_idx, model = "iid", constr = TRUE, hyper = list(prec = pc_prec)) +
  ...
  results <- rbind(results, data.frame(species = species,
                                       ...
                                       spde_range = fit$summary.hyperpar$mean[1],
                                       spde_sd = fit$summary.hyperpar$mean[2],
                                       plotRE_sd = sqrt(var_plt)))
  
  results <- results %>% arrange(sum_est)
  results$species_number <- 1:nrow(results)
  
  saveRDS(results,"../output/empirical_results_RE2.RDS")
...
# ------------------------------------
results <- readRDS("../output/empirical_results_RE2.RDS")
