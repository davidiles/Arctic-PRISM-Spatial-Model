...
for (i in 1:nrow(species_to_run)){
  ...
  prior_range <- c(100, 0.999)        # 99% chance range is smaller than 100km
  ...
  # Controls the 'residual spatial field'.  This can be adjusted to create smoother surfaces.
  prior_range <- c(100, 0.001)        # 1% chance range is smaller than 1000km
  prior_sigma <- c(0.1,0.01)         # 1% chance sd is larger than 0.5
  ...
  comps <- ~
    ...
    count_stratum_1(stratum_1, model = "linear", mean.linear = log(0.01), prec.linear = 1)+
    count_stratum_2(stratum_2, model = "linear", mean.linear = log(0.01), prec.linear = 1)+
    count_stratum_3(stratum_3, model = "linear", mean.linear = log(0.01), prec.linear = 1)+
    count_stratum_4(stratum_4, model = "linear", mean.linear = log(0.01), prec.linear = 1)+
    count_stratum_5(stratum_5, model = "linear", mean.linear = log(0.01), prec.linear = 1)+
    count_stratum_6(stratum_6, model = "linear", mean.linear = log(0.01), prec.linear = 1)+
    count_stratum_7(stratum_7, model = "linear", mean.linear = log(0.01), prec.linear = 1)+
    
    spde_present(geometry, model = matern_present) +
    present_stratum_1(stratum_1, model = "linear", mean.linear = qlogis(0.1), prec.linear = 1)+
    present_stratum_2(stratum_2, model = "linear", mean.linear = qlogis(0.1), prec.linear = 1)+
    present_stratum_3(stratum_3, model = "linear", mean.linear = qlogis(0.1), prec.linear = 1)+
    present_stratum_4(stratum_4, model = "linear", mean.linear = qlogis(0.1), prec.linear = 1)+
    present_stratum_5(stratum_5, model = "linear", mean.linear = qlogis(0.1), prec.linear = 1)+
    present_stratum_6(stratum_6, model = "linear", mean.linear = qlogis(0.1), prec.linear = 1)+
    present_stratum_7(stratum_7, model = "linear", mean.linear = qlogis(0.1), prec.linear = 1)
